...
Development checks require the pinned Playwright dependency and its Chromium binary. Installation needs internet access once; browser tests themselves block all external requests.
...
On a Linux host missing browser system libraries, follow Playwright’s installation diagnostics. No system changes are needed for the game itself. If your environment requires writable caches, set `npm_config_cache` and `PLAYWRIGHT_BROWSERS_PATH` to writable directories for both installation and execution. This project was validated using `/tmp/casebook-browsers` for Chromium.

`npm run check` runs all three checks. No bundler or compilation step is required; the browser loads the JavaScript modules directly. The server serves an explicit allowlist of app assets, binds to loopback, and sets a policy forbidding external connections and inline scripts.
...
- Unit tests compare all atomic predicates against the independent model, exercise malformed saves and contradictory notes, and check hints and accusations for every evidence subset of four cases. Poisoning undiscovered evidence must leave hints unchanged.
...
This is a local-server application, not an installable PWA. Browser validation covers Chromium only. Save/generator formats are versioned; there is no migration for future incompatible versions. Imported JSON is limited to 64 KiB and notes to 2,000 characters. The 30 most recent accusation attempts are retained. Saves are editable local data, not tamper-proof competitive records. No private data, paid compute, or personal integrations are needed.
